...
**版本：** 1.0-draft  
**日期：** 2026-08-09  
**状态：** 已完成 brainstorming 设计确认，等待书面规约复核
...
LLM 只能返回经模式验证的结构化动作，不能直接访问文件系统或 shell。第一版工具集包括：列出目录、读取文件、搜索文本、写入文件、删除或移动文件、查看 Git status/diff、执行参数数组形式的命令、运行验证器、保存或检索记忆，以及声明完成或无法继续。

工具由 `ToolRegistry` 注册并分发。每个工具公开名称、参数模型、风险元数据和执行接口。未知工具、额外字段、类型错误及超限参数在执行前被拒绝。
...
确定性治理策略依次执行动作校验、工具权限检查、路径解析、工作区围栏、敏感路径检查、命令分析和资源限制。结果为：

- `ALLOW`：只读或低风险动作可执行。
- `REQUIRE_APPROVAL`：删除、移动、安装依赖、Git commit、项目脚本等动作暂停等待审批。
- `DENY`：提权、系统破坏、危险 Git 操作、工作区逃逸和敏感凭据访问不可审批。

审批绑定任务、工具、规范化参数、工作区、策略版本、动作摘要和过期时间，使用一次后失效。任何参数变化都必须重新审批。
...
**边界：** 单任务最多 20 轮、连续失败最多 4 次，默认总时限 30 分钟；均可在不突破硬上限的情况下收紧。  
...
**行为：** 返回允许、需审批或禁止的决定及命中规则；审批恢复前重新计算动作摘要。  
**输出：** 治理决定或待审批记录。  
**边界：** 配置不能关闭工作区围栏、敏感路径保护和硬禁令。  
**错误处理：** 策略无法解析时默认拒绝；过期、已消费或摘要不匹配的审批不能执行动作。
...
**输入：** 用户记忆操作、已通过任务摘要、仓库根目录的 `guarded-agent.toml`。  
**行为：** 校验配置；保存可信记忆；检索相关条目。  
**输出：** 有界记忆集合或明确的配置错误。  
**边界：** 配置文件禁止包含凭据；模型推测不自动保存。  
**错误处理：** 无效配置阻止任务启动；数据库写入失败不会被报告为任务完成。
...
| Approval | id, task_id, action_digest, policy_version, status, expires_at, consumed_at | 只可消费一次 |
